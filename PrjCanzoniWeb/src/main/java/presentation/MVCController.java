...
@WebServlet("/provami") //è una route (rotta) - URL endpoint http://localhost:8080/Canzoni/provami -> ci esce la scritta Hey sei connesso! Mi hai chiamato usando GET
public class MVCController extends HttpServlet{
	
	CanzoneService service = new CanzoneServiceImpl();
	
	// chiamata ad una servlet
	@Override
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		
		//1) ci da la risposta
		//response.getWriter().print("Hey sei connesso! Mi hai chiamato usando GET"); 
		//2) reindirizza redirect
		//response.sendRedirect("addCanzone.jsp");
		//3) 
		Random r = new Random();
